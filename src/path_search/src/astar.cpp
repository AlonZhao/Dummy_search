...
//std::cout<<"OKKKKKKKKKKKKKKKKKKK1"<<std::endl;
    ...
    if (reach_end) {
      ...
      has_path_ = true;
//std::cout<<"OKKKKKKKKKKKKKKKKKKK2"<<std::endl;
      return REACH_END;
    }
...
//std::cout<<"OKKKKKKKKKKKKKKKKKKK3"<<std::endl;
    /* ---------- expansion loop ---------- */
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) 
        {
          ...
          if(!map_ptr_->isStateValid(pro_pos))
          {
            continue;
      ...
      NodePtr pro_node =expanded_nodes_.find(pro_id);
          if (pro_node != NULL && pro_node->node_state == IN_CLOSE_SET) {
            ...
            continue;//找到了并且是已经搜索的
          }
//std::cout<<"OKKKKKKKKKKKKKKKKKKK5"<<std::endl;
          /* collision free */
          // double dist = dynamic ?
          // edt_environment_->evaluateCoarseEDT(pro_pos, cur_node->time + dt) :
          //                         edt_environment_->evaluateCoarseEDT(pro_pos,
          //                         -1.0);
          // double dist = edt_environment_->evaluateCoarseEDT(pro_pos, -1.0);//距离
          // if (dist <= margin_) {
          //   continue;
          // }
...
void Astar::init() {
  ...
  //edt_environment_->getMapRegion(origin_, map_size_3d_);//修改 地图初始化
  origin_ = map_ptr_->getOrigin();
  map_size_3d_ = map_ptr_->getMapSize();
...
void Astar::setEnvironment(const env::OccMap::Ptr& env) {
  this->map_ptr_ = env;//修该 地图指针
}
